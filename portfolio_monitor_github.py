...
def get_ai_summary(report_text):
    ...
    payload = {
        "contents": [{
            "parts": [{
                "text": f"你是一位顶级的华尔街对冲基金经理。请阅读以下我今天的跨市场资产监控报告，结合当日宏观市场。用专业、犀利且极其简练的语言（字数不限），总结今天的宏观资金动向，对我的持仓头寸表现给出针对性的精准的建议，对仓位分布的健康性做出评价。并对我今天的实盘盈亏表现给出你的点评。不要说废话，直接给出结论。\n\n今日实盘报告数据如下：\n{report_text}"
            }]
        }],
        "generationConfig": {
